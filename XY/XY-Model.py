...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan  8 22:04:14 2017
...
def SWang(T):
    ...
    for step in np.arange(STEP):
        ...
        Esq_sum += E**2
        Msq_sum += M**2
        sample_n=round(STEP/repeat)
        if(step%sample_n==0):
              config_line=np.reshape(XY,newshape=(1,L*L))
              config_table.append(np.append(config_line,T))
              print('sampling:',step%sample_n)
    ...
    return config_table, E_mean, M_mean, Esq_mean, Msq_mean
...
#    xy=np.array(xy)
#    for xy_list in xy:   
#          config_tables.extend([xy_list])
    config_tables.extend(xy)
np.savetxt('output.data',config_tables)
